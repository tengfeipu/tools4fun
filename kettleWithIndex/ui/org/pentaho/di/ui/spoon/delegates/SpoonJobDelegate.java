      ...
      public void run( IProgressMonitor monitor ) throws InvocationTargetException, InterruptedException {
        try {
          ...
          for ( int i = 0; i < tables.length && !monitor.isCanceled(); i++ ) {
            ...
            sql = JdbcDataMetaUtil.exportPkAndIndex(sourceDbInfo,sourceCon,tables[i],targetDbInfo,targetCon,tables[i]);
            if ( !Utils.isEmpty( sql ) ) {
              String jesqlname = BaseMessages.getString( PKG, "Spoon.RipDB.AddPkAndIndex" ) + tables[i] + "]";
              JobEntrySQL jesql = new JobEntrySQL( jesqlname );
              jesql.setDatabase( targetDbInfo );
              jesql.setSQL( sql );
              jesql
                      .setDescription( BaseMessages.getString( PKG, "Spoon.RipDB.AddPkAndIndex" )
                              + targetDbInfo + "].[" + tables[i] + "]" );
